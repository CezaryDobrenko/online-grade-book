...
class personal_announcement extends REST_Controller{

    //Constructor
    public function __construct(){
        parent::__construct();
        $this->load->model("class_register_api/user/personal_announcement_model");
    ...
    //Read note
    public function read_announcement_get(){
        if(personal_announcement::tokenAccessValidation("Student","Parent")){
            $announcement_data = $this->personal_announcement_model->get_all_announcements();
            if(count($announcement_data) > 0){
                $this->response(array("message" => "Announcement note list", "data" => $announcement_data), parent::HTTP_OK);
            } 
            else {
