...
class staf_absence extends REST_Controller{
    ...
    public function create_absence_post(){
        ...
        if(staf_absence::tokenAccessValidation("Nauczyciel","Dyrektor")){
            ...
            $vfc = isset($data->absence_lesson_number) && isset($data->absence_date)  && isset($data->absence_student_id);
    ...
    public function update_absence_put(){
        ...
        if(staf_absence::tokenAccessValidation("Nauczyciel","Dyrektor")){
            ...
            $data = json_decode(file_get_contents("php://input"));
            $vfc = isset($data->absence_id) && isset($data->absence_lesson_number) && isset($data->absence_date) && isset($data->absence_student_id);
            $headers = $this->input->request_headers();
            $token = $headers['Authorization'];
            $decoded_token = authorization::validateToken($token);
            ...
            if($vfc){  
                if(!empty($this->staf_absence_model->is_student_exists($data->absence_student_id))){
                    if(!empty($this->staf_absence_model->is_teacher_creator($data->absence_id, $decoded_token->data->teacher_id))){
                        ...
                        $this->response(array("message" => "You are not the creator of this absence record"), parent::HTTP_CONFLICT);
                    }
                } else {
                    $this->response(array("message" => "Student does not exist"), parent::HTTP_NOT_FOUND);
                }
