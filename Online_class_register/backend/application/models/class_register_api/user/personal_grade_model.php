...
class personal_grade_model extends CI_Model{
    ...
    public function get_all_grade($student_id){
        $this->db->select('tg.grade_id, tg.grade_value, tg.grade_comment, tg.grade_created_at, ts.subject_name AS grade_subject_name, tgc.grade_category_name, CONCAT( tt.teacher_name, " ", tt.teacher_surname ) AS grade_teacher_fullname');
        $this->db->from("tbl_grades AS tg");
        $this->db->join('tbl_subjects AS ts','ts.subject_id = tg.grade_subject_id');
        $this->db->join('tbl_teachers AS tt','tt.teacher_id = tg.grade_teacher_id');
